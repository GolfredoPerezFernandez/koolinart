...
export default function CardPreview() {
  ...
  let banner_perfil_koolinart =
    "https://ipfs.moralis.io:2053/ipfs/QmWwFxtciV53A3zeWgvztB6Ni3Ck9w2Jg9N4F77CrzHfMz/0xE568887Bf75AeA78147730CC4101aDf09626759E/banner_perfil_koolinart.svg";
  ...
  const nameNft = PreviewNftName ? PreviewNftName : "My name";
  const imageIdentification = CreateNFtImageIdentification
    ? CreateNFtImageIdentification
    : banner_perfil_koolinart;
  const usernameRender = UserRender.attributes?.username
    ? UserRender?.attributes?.username
    : "Not username set";
  const userAvatarRender = UserRender?.attributes?.userAvatar
    ? UserRender?.attributes?.userAvatar
    : icono_user_basic;
  const minimumBid = PreviewNftBids ? PreviewNftBids.toString() : "0";
  const price = PreviewNftPrice ? PreviewNftPrice.toString() : "0";
            ...
            {usernameRender ? (
              <Tooltip title={usernameRender} followCursor>
                <Typography
                  ...
                  onClick={() =>
                    userNameModalGlobal("User Name", usernameRender)
                  }
                >
